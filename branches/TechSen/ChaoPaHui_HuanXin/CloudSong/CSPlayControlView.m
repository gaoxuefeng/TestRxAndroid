...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        [playPauseBtn setImage:[UIImage imageNamed:@"player_play_icon"] forState:UIControlStateSelected];
...
- (void)layoutSubviews
{
    ...
    // 2. 播放暂停按钮frame
    CGFloat playPauseBtnW = TRANSFER_SIZE(30) ;
    CGFloat playPauseBtnH = TRANSFER_SIZE(30) ;
    CGFloat playPauseBtnX = SCREENWIDTH - TRANSFER_SIZE(13) - playPauseBtnW ;
    CGFloat playPauseBtnY = playModeBtnY - TRANSFER_SIZE(6.5) ;
    ...
    CGFloat progressViewH = TRANSFER_SIZE(15) ;
...
- (void)setDuration:(NSTimeInterval)duration
{
    ...
    // 设置播放条长度
    _totalTimeLabel.text = [NSString stringWithFormat:@"/ %02d:%02d", (int)duration / 60, (int)duration % 60, nil];
}

- (void)setPlayTime:(NSTimeInterval)playTime {
    _currentTimeLabel.text = [NSString stringWithFormat:@"%02d:%02d", (int)playTime / 60, (int)playTime % 60, nil];
}
