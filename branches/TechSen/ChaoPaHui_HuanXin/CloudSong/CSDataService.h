 ...
 *  获取首页活动标签数据
 *
 *  @param activityType 活动类型（同城趴，最热趴，明星趴）
 */
//- (void)asyncGetHomeActivityTagListByactivityType:(NSNumber *)activityType startIndex:(NSNumber *)startIndex handler:(void (^)(NSArray *activities))handler ;
 ...
 *  获取首页推荐活动数据
 */
- (void)asyncGetHomeRecommendActivitiesWithStartIndex:(NSNumber *)startIndex handler:(void (^)(NSArray *activities))handler ;
/**
 *  获取首页广告轮播数据
 */
- (void)asyncGetHomeAdBannersDataByHandle:(void(^)(NSArray *banners))handler ;
/**
 *  获取首页活动标签数据
 */
- (void)asyncGetHomeActivityTagsByHandle:(void(^)(NSArray *activityTags))handler ;
