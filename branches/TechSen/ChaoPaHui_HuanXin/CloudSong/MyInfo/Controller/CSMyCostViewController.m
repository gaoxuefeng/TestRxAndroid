...
- (void)setupSubViews
{
    ...
    segmentControl.backgroundColor = WhiteColor_Alpha_6;
    segmentControl.titleColor = HEX_COLOR(0xe0dfe2);
    ...
    segmentControlTopLine.backgroundColor = HEX_COLOR(0x3a2b4d);
    ...
    segmentControlCenterLine.backgroundColor = HEX_COLOR(0x3a2b4d);
    ...
    segmentControlBottomLine.backgroundColor = HEX_COLOR(0x3a2b4d);
...
//    _inProgressTableView.backgroundColor = HEX_COLOR(0x1d1c21);
    _inProgressTableView.backgroundColor = [UIColor clearColor];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (item.orderType.integerValue) {
        ...
        case CSMyCostTypeRoomDish:
        {
            ...
            dishDetailVc.payStatusChanged = ^(CSMyCostDishStatusType payStatus)
            {
                [weakSelf.inProgressDatas removeObject:item];
                [tableView reloadData];
//                item.payState = [NSNumber numberWithInteger:payStatus];
//                [tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
        ...
        case CSMyCostTypeRoom:
        {
            ...
            roomDetailVc.payStatusChanged = ^(CSMyCostStatusType payStatus)
            {
                [weakSelf.inProgressDatas removeObject:item];
                [tableView reloadData];
//                item.payState = [NSNumber numberWithInteger:payStatus];
...
- (void)loadMoreDataWithTableView:(UITableView *)tableView
{
    ...
    if (param.startIndex.integerValue == 0) {
        [SVProgressHUD show];
    }
    ...
    [CSMyInfoHttpTool getCostListWithParam:param success:^(CSMyCostResponseModel *result) {
        ...
        if (result.code == ResponseStateSuccess) {
            ...
            [datas addObjectsFromArray:result.data];
                tableView.footer.hidden = result.data.count < 10;
            [tableView reloadData];
        }else
        {
