...
- (void)setupInputAreaView
{
    ...
    passwordTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"请输入动态密码" attributes:@{NSForegroundColorAttributeName: HEX_COLOR(0x9893a0)}];
    ...
    dynamicPwdButton.backgroundColor = [UIColor colorWithWhite:1.0 alpha:0.06];
    dynamicPwdButton.titleLabel.font = [UIFont systemFontOfSize:TRANSFER_SIZE(14.0)];
    [dynamicPwdButton setTitleColor:HEX_COLOR(0xffffff) forState:UIControlStateNormal];
...
- (void)loginBtnOnClick
{
    ...
    if (_passwordTextField.text.length > 32) {
...
//            [SVProgressHUD showSuccessWithStatus:result.message];
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"动态密码已发送至您的手机，为了您的账户安全，登录后请及时修改密码，感谢您对潮趴汇的支持!" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alertView show];
        }else
        {
            [SVProgressHUD showErrorWithStatus:result.message];
