//
//  CSMessagePictureTableViewCell.m
//  CloudSong
//
...
@interface CSMessagePictureTableViewCell ()
...
@implementation CSMessagePictureTableViewCell
...
//            make.left.equalTo(_circleView.superview).offset(TRANSFER_SIZE(14.0));
        ...
        [_pictureView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(TRANSFER_SIZE(100.0), TRANSFER_SIZE(60.0)));
            make.top.equalTo(_iconView.mas_bottom).offset(TRANSFER_SIZE(8.0));
            make.left.equalTo(_iconView);
//            make.bottom.equalTo(_pictureView.superview).offset(-5.0);
        }];
//        [_contentBackgroundView mas_makeConstraints:^(MASConstraintMaker *make) {
//            make.top.equalTo(_iconView.mas_bottom).offset(8.0);
//            make.left.equalTo(_iconView);
//            make.right.lessThanOrEqualTo(_contentBackgroundView.superview).offset(-12.0);
//            make.bottom.equalTo(_contentBackgroundView.superview).offset(-5.0);
//        }];
//        
...
//    _contentLabel = contentLabel;
    ...
    pictureView.layer.cornerRadius = TRANSFER_SIZE(4.0);
...
{
    static NSString *identifier = @"messagePictureCell";
    CSMessagePictureTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[CSMessagePictureTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    return cell;
}
