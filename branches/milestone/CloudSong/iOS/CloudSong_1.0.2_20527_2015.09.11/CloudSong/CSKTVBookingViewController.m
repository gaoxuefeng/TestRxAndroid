...
- (void)viewDidAppear:(BOOL)animated {
    ...
    //    if (_bldKTVData.count + _dianpingKTVData.count == 0) {//此处修改过
...
- (void)getDistrictData {
    ...
    [_nearButton setTitle:@"全城" forState:UIControlStateNormal];//当网络请求失败，赋初值
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //    else if(_dianpingKTVData.count && _bldKTVData.count==0){
    //        ktv = _dianpingKTVData[indexPath.row];
    //    }
    else{
        if (_dianpingKTVData.count) {
            ktv = _dianpingKTVData[indexPath.row];
        }
    }
