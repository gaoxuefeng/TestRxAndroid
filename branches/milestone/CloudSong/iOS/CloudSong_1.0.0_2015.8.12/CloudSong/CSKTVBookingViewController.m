...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"KTV预订";
...
- (void)configUI {
    [super configUI];
    _buttonsContainer.backgroundColor = Color_Hex_1c_1c_20;
    [_cityChooseButton setBackgroundColor:Color_Hex_1c_1c_20];
    [_nearButton setBackgroundColor:Color_Hex_1c_1c_20];
    [_smartSortButton setBackgroundColor:Color_Hex_1c_1c_20];
    _tableView.backgroundColor = Color_Hex_1c_1c_20;
}
...
- (void)showCityChoosingView:(BOOL)show animated:(BOOL)animated {
    ...
    [UIView animateWithDuration:animated ? .5 : 0
                     ...
                     animations:^{
                         _cityChooseViewOffsetY.offset = show ? TRANSFER_SIZE(25) + ([_cityChoosingController citySelectView].height) : TRANSFER_SIZE(25);
                            [self.view layoutIfNeeded];
                        } completion:nil];
}
...
- (void)setupSubviews {
    ...
    [cityChoosingView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        _cityChooseViewOffsetY = make.bottom.equalTo(self.view.mas_top).offset(buttonContainerHeight);
    }];
    ...
    _buttonsContainer.backgroundColor = HEX_COLOR(0x1c1c20);
    ...
    [[CSDataService sharedInstance] asyncGetKTVDataWithCoordinate:_coordinate
                                                       startIndex:_startIndex
                                                         cityName:_selectedCity
                                                            range:0
                                                     businessName: _nearButton.titleLabel.text
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (_bldKTVData.count&&_dianpingKTVData.count) {
        label.text = section == 0 ? @"宝乐迪KTV" : @"大众点评KTV";
    }else{
        label.text = section == 0 ? @"大众点评KTV" : @"宝乐迪KTV";
    }

    
    // 自定义header View
    UIView *header = [[UIView alloc] init];
    header.backgroundColor = HEX_COLOR(0x151417);
