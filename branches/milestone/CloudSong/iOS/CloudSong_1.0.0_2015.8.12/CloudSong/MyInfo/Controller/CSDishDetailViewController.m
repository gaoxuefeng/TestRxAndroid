...
//    containerBgView.autoresizingMask = UIViewAutoresizingNone;
    ...
    [orderIdCell mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(orderIdCell.superview).offset(23.0);
...
//    CGFloat v_padding = TRANSFER_SIZE(25.0);
    ...
    [priceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.right.bottom.equalTo(priceLabel.superview);
    }];
    ...
    [statusLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(statusLabel.superview.mas_right).offset(iPhone6?-32:-TRANSFER_SIZE(27.0));
        make.centerY.equalTo(statusLabel.superview.mas_top).offset(iPhone6?32:TRANSFER_SIZE(27.0));
    }];
...
- (void)loadData
{
    ...
    [CSMyInfoHttpTool getDishCostDetailWithParam:param success:^(CSRoomDetailResponseModel *result) {
        if (result.code == ResponseStateSuccess) {
            ...
            NSString *statusStr;
            switch ([result.data.payState integerValue]) {
...
//                    _cancelOrPayView.hidden = NO;
                ...
                case CSMyCostStatusTypeRefunding:
                    statusStr = @"退款中";
                    [_paiedBtn setTitle:@"退款中" forState:UIControlStateDisabled];
                    _paiedBtn.enabled = NO;
                    break;
                case CSMyCostStatusTypeInCost:
                    statusStr = @"已消费";
                    [_paiedBtn setTitle:@"已消费" forState:UIControlStateDisabled];
                    _paiedBtn.enabled = NO;
                    break;
...
//    [userDefaults synchronize];
    ...
    NSInteger between =  roomModel.startTime.integerValue - startDate.timeIntervalSince1970;
    if (between >0 && between < 3600) {
        [_paiedBtn setTitle:@"即将开始" forState:UIControlStateDisabled];
        _paiedBtn.enabled = NO;
    }else if (between < 0){
        [_paiedBtn setTitle:@"已消费" forState:UIControlStateDisabled];
        _paiedBtn.enabled = NO;
...
/*
 ...
 UIButton *inCancelButton = [UIButton buttonWithType:UIButtonTypeSystem];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    [CSMyInfoHttpTool cancelOrderWithParam:param success:^(CSBaseResponseModel *result) {
        if (result.code == ResponseStateSuccess) {
            CSCancelRoomViewController *cancelRoomVc = [[CSCancelRoomViewController alloc] init];
