...
//                                                         @"http://192.168.1.226/8f/1f/ad35602f195e7a8e3223cafb32a3.jpg"];
                           ...
                           [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"获取KTV数据失败: %@", [error localizedDescription]]];
                       }];
}
    ...
    [CSBaseHttpTool getWithUrl:[NSString stringWithFormat:@"%@%@", ServiceCloudURL, QueryRoomsByTimeProtocol]
                   ...
                   resultClass:[CSQueryKTVPriceResponse class]
                       success:^(CSQueryKTVPriceResponse *responseObj) {
                           ...
                           [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"获取KTV价格失败: %@", [error localizedDescription]]];
                       }];
}
    ...
    [CSBaseHttpTool getWithUrl:[NSString stringWithFormat:@"%@%@", ServiceCloudURL, VODAllSingSongsProtocol]
                   ...
                   resultClass:[CSAllSingSongsResponse class]
                       success:^(CSAllSingSongsResponse *responseObj) {
                           ...
                           [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"获取大家都在唱的歌曲数据失败: %@", [error localizedDescription]]];
                       }];
}
    ...
    [CSBaseHttpTool getWithUrl:[NSString stringWithFormat:@"%@%@", ServiceCloudURL, VODRecommendedAlbumsProtocol]
                   ...
                   resultClass:[CSRecommendedAlbumsResponse class]
                       success:^(CSRecommendedAlbumsResponse *responseObj) {
                           ...
                           [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"获取推荐专辑数据失败: %@", [error localizedDescription]]];
                       }];
}
...
- (void)asyncSelectSong:(CSSong *)song forcely:(BOOL)forcely handler:(void(^)(BOOL success))handler {
    ...
    CSMyRoomInfoModel * roomInfo = GlobalObj.myRooms.firstObject;
    requst.reserveBoxId = roomInfo.reserveBoxId;
    requst.isForce = [NSNumber numberWithBool:forcely];
...
- (NSDictionary *)allCities {
    ...
    @"I":@[@"iPhone", @"iOS", @"iMac", @"iTouch" , @"iPad", @"iWatch", @"iPod", @"iTV", @"iCar"] ,
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1) {
        _isCancel=NO;
        [self asyncSelectSong:_selectingSong forcely:@YES handler:_selectingSongHandler];
    }else{
        _isCancel=YES;
        [self asyncSelectSong:_selectingSong forcely:@YES handler:_selectingSongHandler];
    }
...
- (void)asyncGetHomeActivityDataByactivityType:(NSNumber *)activityType startIndex:(NSNumber *)startIndex handler:(void (^)(NSArray *activities))handler{
    ...
    [CSBaseHttpTool getWithNetWarningUrl:[NSString stringWithFormat:@"%@%@", ServiceCloudURL, HomeActivitiesProtocol]
                                   param:request
