...
@implementation CSVODAllSingingCell
...
- (void)addBtnPressed {
    if ([self.delegate respondsToSelector:@selector(allSingingCell:didSelectSong:)]) {

        _addSongBut.enabled = _addSongBut.selected ? YES :NO;

        [self.delegate allSingingCell:self didSelectSong:_song];
    }
}
...
- (void)setupSubviews {
    ...
    _icon.text=@"1";
    _icon.font=[UIFont systemFontOfSize:12.0];
    _icon.textColor = [UIColor colorWithRed:255 green:255 blue:255 alpha:0.35];
    ...
    _title.textColor = [HEX_COLOR(0x9799a1) colorWithAlphaComponent:.8];
    [_title mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_icon.mas_right).offset(TRANSFER_SIZE(10));
        make.right.equalTo(_title.superview).offset(TRANSFER_SIZE(-50));
        make.top.equalTo(_title.superview).offset(TRANSFER_SIZE(18));
    ...
    _subTitle.textColor = [HEX_COLOR(0x9799a1) colorWithAlphaComponent:.4];
    [_subTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_icon.mas_right).offset(TRANSFER_SIZE(10));
        make.right.equalTo(_subTitle.superview).offset(TRANSFER_SIZE(-50));
