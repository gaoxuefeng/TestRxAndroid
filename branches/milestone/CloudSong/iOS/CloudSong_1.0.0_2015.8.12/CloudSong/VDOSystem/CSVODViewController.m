...
- (void)notificationUpdate:(NSNotification * )notification{
    ...
    if (userInfoDic) {
        ...
        [identArray removeObject:[userInfoDic objectForKey:@"singingId"]];
...
- (void)setupSubviews {
    ...
    _segmentControl.backgroundColor = HEX_COLOR(0x151417);
...
- (void)selectSongPlatformControllerDidPressSingerButton:(CSSelectSongPlatformController *)controller {
    ...
    [self.navigationController pushViewController:[[CSSingersTableViewController alloc] initWithStyle:UITableViewStyleGrouped] animated:YES];
}
