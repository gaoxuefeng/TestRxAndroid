...
//    CSLog(@"加密串 == %@",str);
    // 发送请求和处理
    [manager GET:url parameters:@{@"param":str} success:^(AFHTTPRequestOperation *operation, id responseObject) {
        if (success) {
...
//            CSLog(@"解密文 == %@",decryptedString);
            ...
            NSError *dicError;
            NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:decrypted options:NSJSONReadingMutableContainers error:&dicError];
            NSLog(@"dic = %@",dic);
            success(dic);
...
+ (void)post:(NSString *)url params:(NSDictionary *)params success:(void(^)(id responseObj))success failure:(void(^)(NSError *error)) failure
{
    ...
    // 发送请求和处理
    [manager POST:url parameters:@{@"param":str} success:^(AFHTTPRequestOperation *operation, id responseObject) {
        if (success) {
            NSString *responseString = [[NSString  alloc] initWithData:responseObject encoding:NSUTF8StringEncoding];
            NSData *responseData = [responseString base64DecodedData];
            NSError *error;
