 ...
 * 
 */
public class MyBottomRoundImageView extends ImageView {
	...
	public static final int TYPE_HALF_BOTTOM = 4;// 半椭圆形
	private static final int DEFAULT_ROUND_BORDER_RADIUS = 10;// 默认圆角大小

	// 构造方法
	public MyBottomRoundImageView(Context context) {
		this(context, null);
	}

	public MyBottomRoundImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
		// TODO Auto-generated constructor stub

		mPaint = new Paint();
		mPaint.setAntiAlias(true);// 设置消除锯齿
		mType = TYPE_HALF_BOTTOM;
	...
	protected void onDraw(Canvas canvas) {
		...
		if ((bmp == null || bmp.isRecycled()) && getDrawable() != null) {
			...
			if (null != drawable) {
				...
				} else if (mType == TYPE_ROUND || mType == TYPE_OVAL || mType == TYPE_HALF_BOTTOM) {// 如果是圆角矩形或椭圆
	...
	private Bitmap getDrawBitmap() {
		...
		} else if (mType == TYPE_HALF_BOTTOM) {
			canvas.drawRoundRect(new RectF(0, 0, getWidth(), getHeight() + mRoundBorderRadius), mRoundBorderRadius, mRoundBorderRadius, paint);
		}
