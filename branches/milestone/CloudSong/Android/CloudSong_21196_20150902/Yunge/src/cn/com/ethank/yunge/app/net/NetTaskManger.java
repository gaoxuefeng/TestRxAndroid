...
public class NetTaskManger {
	...
	private int addTaskInfo_Api(String[] apis, String timeKeeperKey) {
		return addTaskInfo(new String[] { HOST_NORMAL, HOST_TEST }, apis, timeKeeperKey, new int[] { TimeUtil.ONE_DAY_MILLIS, TimeUtil.TEN_SECOND_MILLIS });
	}

	/**
	 * hosts[0]: official host hosts[1]: test host
	 */
	private int addTaskInfo(String[] hosts, String[] apis, String timeKeeperKey, int[] updateIntervalMillises) {
		String host = isDebug ? hosts[1] : hosts[0];
		String api = isDebug ? apis[1] : apis[0];
		int updateIntervalMillis = isDebug ? updateIntervalMillises[1] : updateIntervalMillises[0];
		mTaskInfoList.add(new NetworkTaskInfo(new HostApiInfo(host, api), timeKeeperKey, updateIntervalMillis));
		return mTaskInfoList.size() - 1;
	}

	private NetTaskManger() {
		Statistic = addTaskInfo(new String[] { "192.168.1.223:8010", "192.168.1.223:8010" }, new String[] { "blank.gif", "blank.gif" }, "ETHANK_STATISTIC", new int[] { TimeUtil.ONE_DAY_MILLIS,
				TimeUtil.TEN_SECOND_MILLIS });
