...
public class PictureManager {
	...
	//
	// public Bitmap getmenuLogo(String url, boolean isPermanent,
	// OnPictureDownloadFinished opf) {
	...
	public Bitmap getmenuLogo(final View view, final String url, boolean isPermanent, OnPictureDownloadFinished opf, final int defaultImgResource) {
		...
		try {
			...
			if (item != null) {
				...
				item = new PicItem();
				menuLogoMap.put(url, item);
				if (merchantLogoAlreadyExist(url, isPermanent) && (result = loadLogoPic(url, isPermanent)) != null) {
					item.logo = new WeakReference<Bitmap>(result);
					item.status = TYPE_DOWNLOADED;
				} else {
					getPicDownloadExecutor().execute(new DownloadMenuLogoTask(url, isPermanent, opf, new RequestCallBack() {
