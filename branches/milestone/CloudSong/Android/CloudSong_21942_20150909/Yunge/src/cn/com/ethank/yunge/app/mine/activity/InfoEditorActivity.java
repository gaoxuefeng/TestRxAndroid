...
public class InfoEditorActivity extends BaseActivity implements OnClickListener {
	...
	private void hideSoft() {
		...
		imm.showSoftInput(edit_age,InputMethodManager.SHOW_FORCED);  
		imm.hideSoftInputFromWindow(edit_age.getWindowToken(), 0); //强制隐藏键盘 
	}
	...
	protected void onResume() {
		...
		if (result != null) {
			...
			if (!TextUtils.isEmpty(Constants.getImageUrl())) {
				bitmapUtils.display(edit_rivx_img, Constants.getImageUrl());
			} else if (!TextUtils.isEmpty(userInfo.getData().getUserInfo().getHeadUrl())) {
				bitmapUtils.display(edit_rivx_img, userInfo.getData().getUserInfo().getHeadUrl());
			} else {
				if (gender != null && "女".equals(userInfo.getData().getUserInfo().getGender())) {
					edit_rivx_img.setBackground(getResources().getDrawable(R.drawable.mine_default_avatar_female));
				} else {
					edit_rivx_img.setBackground(getResources().getDrawable(R.drawable.mine_default_avatar_man));
				}
			}
			...
			if(userInfo.getData().getUserInfo().getAge() != 0){
			...
			nickname_et_delete.setText(edit_your_nickname.getText());
			nickname_et_delete.setSelection(edit_your_nickname.getText().length());
		}
	...
	private void initView() {
		...
		head_tv_title.setFocusable(true);
		head_tv_title.setFocusableInTouchMode(true);
		head_tv_title.requestFocus();

		edit_your_nickname.setOnClickListener(this);
		...
		StringUtil.setEditContentlimit(new EditTextCallBack() {
			...
			public void after(String result) {
				...
				if (!TextUtils.isEmpty(nickName) && !nickName.equals(result.toString())) {
					head_tv_right.setTextColor(getResources().getColor(R.color.order_pink));
					isColor = true;
				} else {
					head_tv_right.setTextColor(getResources().getColor(R.color.white_right));
					isColor = false;
				}*/
	...
	public void onClick(View v) {
		...
		case R.id.head_tv_right:
			...
			String nickName = edit_your_nickname.getText().toString().trim();
			...
			map.put("age", edit_your_age.getText() + "");
			map.put("bloodType", edit_your_blood.getText() + "");
			map.put("constellation", edit_your_constell.getText() + "");
			map.put("gender", edit_your_gender.getText() + "");
		...
		case R.id.edit_rl_head_img:
			...
			break;
		case R.id.edit_your_nickname:
			edit_your_nickname.setSelection(edit_your_nickname.getText().toString().length());
			break;
		...
		case R.id.gender_boy:
			edit_your_gender.setText("男");
			/*if ("男".equals(userInfo.getData().getUserInfo().getGender())) {
				head_tv_right.setTextColor(getResources().getColor(R.color.white_right));
				//isColor = false;
			} else {
				head_tv_right.setTextColor(getResources().getColor(R.color.order_pink));
		...
		case R.id.gender_girl:
			edit_your_gender.setText("女");
			/*if ("女".equals(userInfo.getData().getUserInfo().getGender())) {
				//isColor = false;
				head_tv_right.setTextColor(getResources().getColor(R.color.white_right));
			} else {
				head_tv_right.setTextColor(getResources().getColor(R.color.order_pink));
		...
		case R.id.edit_introduce:
			...
			introduce.putExtra("editor", "editor");
		...
		case R.id.nickname_confirm:
			...
			ProgressDialogUtils.show(this);
			// new ModifyInfoTask(nickname, window, edit_your_nickname,
			// nickname_et_delete.getText().toString()).run();
	...
	void CreatePopu(String flag) {
		...
		int height = UICommonUtil.dip2px(getApplicationContext(), 400);
		...
		mPopupWindow.update();
		mPopupWindow.showAtLocation(info_ll_parent, Gravity.BOTTOM, 0, 0);
	}
	...
	private class DateArrayAdapter extends ArrayWheelAdapter<String> {
		...
		protected void configureTextView(TextView view) {
			...
			view.setLayoutParams(new LayoutParams(android.view.ViewGroup.LayoutParams.WRAP_CONTENT, height));
