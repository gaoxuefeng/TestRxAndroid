...
public class DemandedFragment extends BaseFragment {
		...
		try {
			if (!Constants.isBinded()) {
				if (isVisible())
					...
					mrlv_demanded_songs.onRefreshComplete();
				return;
			}
	...
	private void initView(View view) {
		...
		IntentFilter filter=new IntentFilter("nextsongupdate");
		BroadcastReceiver receiver = new UpdateNextSongReciver();
		context.registerReceiver(receiver , filter);
	}
	
	public class UpdateNextSongReciver extends BroadcastReceiver{

		@Override
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equals("nextsongupdate")) {
				getNetData(true);
			}
