...
public class MyRoundImageView extends ImageView {
		...
		try {
			...
			if ((bmp == null || bmp.isRecycled()) && getDrawable() != null) {
				...
				if (null != drawable) {
					...
					int scaleWidth = (int) (scale * dwidth);
					int scaleHeight = (int) (scale * dheight);
					int sdh = (scaleHeight - getHeight()) / 2;
					int sdw = (scaleWidth - getWidth()) / 2;
					drawable.setBounds(-sdw, -sdh, scaleWidth - sdw, scaleHeight - sdh);
					// drawable.setBounds(0, 0, scaleWidth, scaleHeight);

					drawable.draw(drawCanvas);
					// 获取bitmap，即圆形、圆角或椭圆的bitmap
					if (mMaskBitmap == null || mMaskBitmap.isRecycled()) {
						mMaskBitmap = getDrawBitmap(scaleWidth, scaleHeight, sdw, sdh);
					}
	...
	private Bitmap getDrawBitmap(int scaleWidth, int scaleHeight, int sdw, int sdh) {
		Bitmap bitmap = Bitmap.createBitmap(getWidth(), getHeight(), Bitmap.Config.ARGB_8888);
		Canvas canvas = new Canvas(bitmap);
		Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
		paint.setColor(Color.BLACK);
