...
public class DefaultNetworkStatusService implements INetworkStatusService {
	private static final long DEFAULT_WIFI_RECENT_INTERVAL = 7 * 24 * 60 * 60 * 1000; // 7
																						// days
	...
	public DefaultNetworkStatusService(long wifiRecentIntervalMillis, boolean isTelcom) {
	...
	private NetworkStatusImp createCurrentNetworkStatus() {
		ISettingService setting = (ISettingService) CoyoteSystem.getCurrent().getService(ISettingService.class);
		final long lastWifiTime = setting.getLong(LAST_CONNECT_WIFI_STAMP, 0);
		return new NetworkStatusImp(CoyoteSystem.getCurrent().getAppContext(), System.currentTimeMillis() - lastWifiTime < mWifiRecentInterval,
				mIsTelcom);
	}
	...
	public void notifyNetworkStatusChange(boolean isBreak) {
		for (OnNetworkChangedListener listener : mListeners) {
			listener.onNetworkConnectChanged(!isBreak);
		}
		if (isBreak) {
			return;
		}
		...
		if (mSingletonTaskScheduler == null) {
			mSingletonTaskScheduler = new SingletonTaskScheduler(new Callable<Boolean>() {

				@Override
				public Boolean call() throws Exception {
					NetworkStatusImp ns = createCurrentNetworkStatus();
					if (ns.wifiAvailable()) {
						getSetting().setLong(LAST_CONNECT_WIFI_STAMP, System.currentTimeMillis());
					}
					for (OnNetworkChangedListener listener : mListeners) {
