...
public class MerchantDetailFragment extends BaseFragment implements OnClickListener {

	protected static long serviceTime = 0;

	// TODO 修改了东西琨
	public ImageHandler handler = new ImageHandler(null);
	// new ImageHandler(new WeakReference<MerchantDetailFragment>(this));
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		view = inflater.inflate(R.layout.fragment_merchant_detail, container, false);

		merchant_view = view.findViewById(R.id.merchant_view);
		 ...
		 * 预订的时间
		 */
		pre_detail_tv_day = (TextView) view.findViewById(R.id.pre_detail_tv_day);
		pre_detail_tv_hour = (TextView) view.findViewById(R.id.pre_detail_tv_hour);
		pre_detail_tv_duration = (TextView) view.findViewById(R.id.pre_detail_tv_duration);

		// --拨打电话
		detail_ll_phone = (LinearLayout) view.findViewById(R.id.detail_ll_phone);
		detail_ll_phone.setOnClickListener(this);

		CreatePopu();
		detail_lv_box = (ListView) view.findViewById(R.id.detail_lv_box);

		// --拨打电话弹出提示框
		pop_utils = View.inflate(getActivity(), R.layout.pop_utils, null);
	...
	private RelativeLayout onViewPager() {
		RelativeLayout relativeLayout = (RelativeLayout) view.findViewById(R.id.detail_rl_head);
		...
		for (int i = 0; i < imageUrls.length; i++) {
			...
			LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(UICommonUtil.dip2px(getActivity(), 18), UICommonUtil.dip2px(getActivity(), 2));
			layoutParams.setMargins(0, 0, UICommonUtil.dip2px(getActivity(), 6), UICommonUtil.dip2px(getActivity(), 6));
		...
		RelativeLayout layout = (RelativeLayout) view.findViewById(R.id.merchant_head);
		TextView head_tv_left = (TextView) layout.findViewById(R.id.head_tv_left);
		head_tv_left.setVisibility(View.VISIBLE);
		head_tv_left.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				setMainTabPosition(1);
			}
		});
		...
		vp.setAdapter(new ViewPagerAdapter(views, getActivity(), imageUrls));
		...
		ListView item_lv_group = (ListView) view.findViewById(R.id.item_lv_group);
		...
		detail_rl_pre_time = (LinearLayout) view.findViewById(R.id.detail_rl_pre_time);
						...
						if (boxInfo2.getCode() != 0) {
							...
							if (VerifyStringType.isFloatOrNum(serviceTimeStr)) {
								try {
									...
									MerchantDetailFragment.serviceTime = serviceTime;
								} catch (Exception e) {
									e.printStackTrace();
							 ...
							 */
							boxListAdapter = new BoxListAdapter(getActivity(), boxInfos, homeInfo, pre_detail_tv_day, pre_detail_tv_duration, pre_detail_tv_hour);
	...
	public void onClick(View v) {
		...
		case R.id.detail_ll_phone:
			PopupWindowUtils.show(getActivity(), pop_utils, view, false);
			break;
				...
				if (!TextUtils.isEmpty(result)) {
					...
					if (orderInfo != null) {
						...
						if (orderInfo.getCode() == 0) {
							...
							// --跳转到订单支付界面--
							Intent order = new Intent(getActivity(), OrderFormActivity.class);
							startActivity(order);
						} else {
	...
	void CreatePopu() {
		...
		int lineHeight = UICommonUtil.dip2px(getActivity(), 80);
		WheelViewConstantUtils.setChildView(0, false, 6, lineHeight);
		View popview = LayoutInflater.from(getActivity()).inflate(R.layout.mywheelview, null, true);// 弹出窗口包含的视图

		initView(popview);

		int width = UICommonUtil.dip2px(getActivity(), 300);
		int height = UICommonUtil.dip2px(getActivity(), 300);
		...
		// 设置弹出窗口的背景
		ColorDrawable cd = new ColorDrawable(0x00999999);
	...
	void initView(View view) {
		...
		hoursArrayAdapter = new DateArrayAdapter(getActivity(), hours, 0);
		wheelview2_id.setViewAdapter(hoursArrayAdapter);
		wheelview2_id.setCurrentItem(0);
		updateHourss(wheelview2_id);

		// addFiveItem();
		timesArrayAdapter = new DateArrayAdapter(getActivity(), times, 0);
		...
		dayArrayAdapter = new DayArrayAdapter(getActivity(), calendar);
	...
	void TvOnclickMethod(final TextView tv) {
		// String[] time1 = { "", "一", "二", "三", "四", "五", "六", "七", "八", "九",
		...
		tv.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				if (tv == tv_ok_id) {
					Toast.makeText(getActivity(), daysParams + "---" + clocksParams + "----" + timesParams, 1).show();
		...
		wheelview.addChangingListener(new OnWheelChangedListener() {
			...
			public void onChanged(WheelView wheel, int oldValue, int newValue) {
				...
				if (wheelview == wheelview1_id) {
					...
					timesArrayAdapter = new DateArrayAdapter(getActivity(), times, newValue);
					wheelview3_id.setViewAdapter(timesArrayAdapter);
				}
			}
		});
	...
	private void setWheelView2Adapter(int newValue) {
		...
		hoursArrayAdapter = new DateArrayAdapter(getActivity(), hours, newValue);
		wheelview2_id.setViewAdapter(hoursArrayAdapter);
	...
	private class DayArrayAdapter extends AbstractWheelTextAdapter {
		...
		public View getItem(int position, View cachedView, ViewGroup parent) {
			...
			view.getLayoutParams().height = UICommonUtil.dip2px(getActivity(), 40);
			view.setPadding(0, UICommonUtil.dip2px(getActivity(), 4), 0, 0);
		...
		private void getPreTime2() {
			...
			for (int i = 0; i < hours.length; i++) {
				if (MerchantDetailFragment.hour.equals(hours[i])) {
					oneIndex = i;
	...
	private class DateArrayAdapter extends ArrayWheelAdapter<String> {
		...
		protected void configureTextView(TextView view) {
			...
			int weight = UICommonUtil.dip2px(getActivity(), 40);
	...
	public void OnFragmentResume() {
		...
		if (homeInfo != null) {
			pre_detail_tv_showName = (TextView) view.findViewById(R.id.pre_detail_tv_showName);
			...
			pre_detail_tv_fen = (TextView) view.findViewById(R.id.pre_detail_tv_fen);
			pre_detail_tv_fen.setText(homeInfo.getRating() / 10 + "分");

			pre_detail_rb_start = (RatingBar) view.findViewById(R.id.pre_detail_rb_start);
			pre_detail_rb_start.setRating((float) (homeInfo.getRating() / 10));

			detail_tv_address = (TextView) view.findViewById(R.id.detail_tv_address);
	...
	// private void addFiveItem() {
	// String[] data = new String[times.length + 1];
	// data[0] = "5分钟";
