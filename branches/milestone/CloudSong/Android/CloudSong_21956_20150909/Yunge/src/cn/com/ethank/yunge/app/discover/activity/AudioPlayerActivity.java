...
public class AudioPlayerActivity extends BaseActivity implements OnClickListener {
	...
	private void setviewData() {
		try {
			...
			try {
				MyImageLoader.displayImage(iv_useicon, musicBean.getAvatarUrl(), R.drawable.mine_defaultavatar);
			} catch (Exception e) {
			}
	...
	private void addUserList(List<String> urls) {
		...
		for (int i = 0; i < sum; i++) {
			...
			if (sum > 0) {
				MyImageLoader.displayImage(view, urls.get(i).toString(), R.drawable.mine_defaultavatar);
			} else {
