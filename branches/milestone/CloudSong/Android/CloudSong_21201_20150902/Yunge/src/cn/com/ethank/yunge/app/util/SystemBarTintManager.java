...
public class SystemBarTintManager {
	...
	public SystemBarTintManager(Activity activity) {
		...
		/*
		 * if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) { // check
		 * theme attrs int[] attrs = {android.R.attr.windowTranslucentStatus,
		 * android.R.attr.windowTranslucentNavigation}; TypedArray a =
		 * activity.obtainStyledAttributes(attrs); try { mStatusBarAvailable =
		 * a.getBoolean(0, false); mNavBarAvailable = a.getBoolean(1, false); }
		 * finally { a.recycle(); }
		 * 
		 * // check window flags WindowManager.LayoutParams winParams =
		 * win.getAttributes(); int bits =
		 * WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS; if
		 * ((winParams.flags & bits) != 0) { mStatusBarAvailable = true; } bits
		 * = WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION; if
		 * ((winParams.flags & bits) != 0) { mNavBarAvailable = true; } }
		 */
