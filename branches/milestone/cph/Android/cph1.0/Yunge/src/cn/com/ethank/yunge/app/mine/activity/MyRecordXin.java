			...
			new BackgroundTask<String>() {
				...
				protected String doWork() throws Exception {
					return HttpUtils.getJsonByPost(
							Constants.hostUrlCloud + Constants.myRecord, hashMap)
							.toString();
				}
				...
				protected void onCompletion(String result, Throwable exception,
					...
					if (result != null) {
						RecordInfoXin recordInfoXin = JSON.parseObject(result,
								RecordInfoXin.class);
						...
						if (recordInfoXin.getCode() == 0) {
							if (recordInfoXin.getData().size() == 0) {
								...
								for (int i = 0; i < recordInfoXin.getData()
										.size(); i++) {
...
//		recordAdapter.setList(records);
	...
	Runnable updateThread = new Runnable() {
		public void run() {
			...
			if (player != null) {
				String progresstime = new SimpleDateFormat("mm:ss")
	...
	private void initView() {
		...
		mrlv_record.setPullToRefreshEnabled(false);
...
//		LinearLayout ll_share1 = (LinearLayout) contentView.findViewById(R.id.ll_share1);
//		ImageView iv_QQ_xin = (ImageView) contentView.findViewById(R.id.iv_QQ_xin);
//		ImageView iv_sina_xin = (ImageView) contentView.findViewById(R.id.iv_sina_xin);
//		iv_QQ_xin.setOnClickListener(this);
//		iv_sina_xin.setOnClickListener(this);
		

		// 取消
		TextView tv_cancle = (TextView) contentView
				.findViewById(R.id.tv_cancle);
		// 微信
		ImageView iv_wechat = (ImageView) contentView
				.findViewById(R.id.iv_wechat);
		// 朋友圈
		ImageView iv_friend_cricle = (ImageView) contentView
				.findViewById(R.id.iv_friend_cricle);
		// QQ
...
//		View friend_circle_view = contentView.findViewById(R.id.friend_circle_view);
