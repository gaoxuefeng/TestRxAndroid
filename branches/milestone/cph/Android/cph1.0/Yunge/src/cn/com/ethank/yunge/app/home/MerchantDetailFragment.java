...
public class MerchantDetailFragment extends BaseFragment implements OnClickListener {
	...
	public ImageHandler handler = new ImageHandler(new WeakReference<MerchantDetailFragment>(this));
	...
	// new String[] { "http://image.zcool.com.cn/56/35/1303967876491.jpg",
	...
	private RelativeLayout onViewPager() {
		...
		for (int i = 0; i < imageUrls.length; i++) {
			...
			LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(UICommonUtil.dip2px(getActivity(), 18), UICommonUtil.dip2px(
					getActivity(), 2));
		...
		if (imageUrls.length > 0) {
			...
			ImageView imageView = new ImageView(getActivity());
			imageView.setScaleType(ScaleType.FIT_XY);
			imageView.setBackground(getResources().getDrawable(R.drawable.schedule_defaul_head_img));
			views.add(imageView);
		}
			...
			protected void onCompletion(String result, Throwable exception, boolean cancelled) {
				if (result != null) {
					...
					if (boxInfo2.getCode() != 0) {
						 ...
						 */
						boxListAdapter = new BoxListAdapter(getActivity(), boxInfos, homeInfo, pre_detail_tv_day, pre_detail_tv_duration,
								pre_detail_tv_hour);
