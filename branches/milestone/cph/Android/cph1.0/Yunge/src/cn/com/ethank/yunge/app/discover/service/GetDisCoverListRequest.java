			...
			protected List<DiscoverInfo> doInBackground(Void... params) {
				...
				if (result != null && !result.isEmpty()) {
					...
					if (bean.getCode() == 0 && bean.getData() != null && !bean.getData().isEmpty()) {
						...
						// DisCoverConstantUtils utils = new
						// DisCoverConstantUtils();
						// for (int i = 0; i < list.size(); i++) {
						// String url = list.get(i).getMusicUrl();
						// if (!utils.musicUrlList.contains(url)) {
						// utils.musicUrlList.add(list.get(i).getMusicUrl());
						// utils.musicNameList.add(list.get(i).getMusicName());
						// }
						// }
						return list;
					} else {
