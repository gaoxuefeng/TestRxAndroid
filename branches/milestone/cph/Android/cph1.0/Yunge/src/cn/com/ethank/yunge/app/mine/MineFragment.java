...
public class MineFragment extends BaseFragment implements View.OnClickListener {
	...
	private void initData() {
		...
		if (!TextUtils.isEmpty(Constants.getToken())) {
			...
			if (!TextUtils.isEmpty(userInfo.getData().getUserInfo().getNickName())) {
				...
				/*
				 * Drawable drawable =
				 * getResources().getDrawable(R.drawable.mine_edit_icon); //
				 * 这一步必须要做,否则不会显示. drawable.setBounds(0, 0,
				 * drawable.getMinimumWidth(), drawable.getMinimumHeight());
				 * frag_name.setCompoundDrawables(null, null, drawable, null);
				 */
			}

			// String imageUrl =
			// SharePreferencesUtil.getStringData(SharePreferenceKeyUtil.imageUrl);
			if (!TextUtils.isEmpty(headUrl)) {
				...
				if (gender != null && gender.equals("女")) {
					...
					drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());
					frag_constel.setCompoundDrawables(drawable, null, null, null);
					mine_iv.setBackgroundResource(R.drawable.mine_defaultavatar);
				}
			...
			mine_iv.setImageResource(R.drawable.mine_defaultavatar);
	...
	public void onClick(View view) {
		...
		case R.id.mine_rl_box:
			...
			break;
		case R.id.mine_view:
			Intent people = new Intent(getActivity(), PeopleHomePageActivity.class);
